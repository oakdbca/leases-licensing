...
def docx_replace_regex(doc_obj, regex , replace, key, bold=False,italic=False, underline=False, **kwargs):
    """
    Replaces occurences of `key` in document paragraphs with value `replace`
    """
...
def create_approval_doc(approval):
    approval_buffer = _create_approval_dummy(approval)
...
def _create_approval_dummy(approval):
    """
    ...
    docx_replace_regex(doc, re.compile(rf"{key}") , f"{proposal.applicant_name}", key, True, False, False)
    # Licence number example italic  (table cell)
    key = "{{lodgement_number}}"
    docx_replace_regex(doc, re.compile(rf"{key}") , f"{approval.lodgement_number}", key, False, True, False)
    # Start/End date examples underlined (no table, inline replacement)
    key = "{{start_date}}"
    start_date = approval.start_date.strftime('%a %d %b %Y, %I:%M %p')
    docx_replace_regex(doc, re.compile(rf"{key}") , f"{start_date}", key, False, False, True)
    key = "{{expiry_date}}"
    expiry_date = approval.expiry_date.strftime('%a %d %b %Y, %I:%M %p')
    docx_replace_regex(doc, re.compile(rf"{key}") , f"{expiry_date}", key, False, False, True)
