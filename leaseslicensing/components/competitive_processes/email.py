...
def send_competitive_process_create_notification(request, competitive_process, **kwargs):
    ...
    bcc_emails = re.split("[\s,;]+",  bcc_email_str) if bcc_email_str else []
    ...
    msg = email.send(email_user_emails,
               cc=cc_emails,
               bcc=bcc_emails,
               context=context)
...
def _log_competitive_process_email(
        email_message, competitive_process, sender=None
    ):
    from leaseslicensing.components.competitive_processes.models import CompetitiveProcessLogEntry
    ...
    if isinstance( email_message, (EmailMultiAlternatives, EmailMessage) ):
        ...
        if hasattr(competitive_process, "originating_proposal"):
            to = retrieve_email_user(competitive_process.originating_proposal.submitter).email
        else:
            to = None
        fromm = smart_text(sender) if sender else f"{settings.SYSTEM_NAME} (Automated Message)"
        all_ccs = ""
