...
export default {
    ...
    created: async function () {
        ...
        vm.$nextTick(() => {
            ...
            // Automatically select the mandatory document types
            vm.selectedDocumentTypes = vm.mandatoryDocumentTypes;

            // Add any other document types that have already been selected
            if (vm.approval.selected_document_types) {
                for (const docType of vm.approval.selected_document_types) {
                    const found = vm.selectedDocumentTypes.some(
                        (el) => el.id === docType.id
                    );
                    if (!found) {
                        vm.selectedDocumentTypes.push(docType);
                    }
                }
            }
        });
