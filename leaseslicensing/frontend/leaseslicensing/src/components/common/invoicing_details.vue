<template>
    <div class="row mb-4">
        <label class="col-form-label col-sm-4">Lease or Licence Charge Method</label>
        <div class="col-sm-8">
            <ul class="list-group">
                <li v-for="charge_method in charge_methods" class="list-group-item">
                    <input v-if="invoicing_details" type="radio" class="form-check-input me-2" name="charge_method"
                        :id="charge_method.key" :value="charge_method.id" v-model="invoicing_details.charge_method" />
    ...
    <div v-if="show_once_off_charge_amount" class="row mb-2">
        <div class="col-sm-4">
            <label for="once_off_charge_amount" class="control-label">Once-off Charge ($AUD)</label>
        </div>
        <div class="col-sm-8">
            <input v-if="invoicing_details" type="number" min="0" step="100" id="once_off_charge_amount"
                class="form-control" v-model="invoicing_details.once_off_charge_amount" required>
        </div>
    </div>
    <div v-if="show_base_fee" class="row mb-4">
        <label for="base_fee_amount" class="col-form-label col-sm-4">Base Fee ($AUD)</label>
        <div class="col-sm-8">
            <input v-if="invoicing_details" type="number" min="0" step="100" id="base_fee_amount" class="form-control"
                v-model="invoicing_details.base_fee_amount">
        </div>
    </div>
    <div v-if="show_fixed_annual_increment">
        <AnnualIncrement v-if="invoicing_details" increment_type="annual_increment_amount"
            :years_array="invoicing_details.annual_increment_amounts" />
    </div>
    <div v-if="show_fixed_annual_percentage">
        <AnnualIncrement v-if="invoicing_details" increment_type="annual_increment_percentage"
            :years_array="invoicing_details.annual_increment_percentages" />
    </div>
    <div v-if="show_percentage_of_gross_turnover">
        <AnnualIncrement v-if="invoicing_details" increment_type="gross_turnover_percentage"
            :years_array="invoicing_details.gross_turnover_percentages" />
        <div class="row mb-2">
            <div class="col-sm-12">
                <BootstrapAlert>
                    The approval holder will be asked to upload their audited financial statement once a year.
                </BootstrapAlert>
            </div>
        </div>
    </div>
    <div v-if="show_review_of_base_fee" class="row mb-2">
        ...
        <div class="col-sm-2">
            <input v-if="invoicing_details" type="number" min="0" max="5" step="1" id="review_once_every"
                class="form-control" v-model="invoicing_details.review_once_every">
        </div>
        <div class="col-sm-2">
            <div v-for="repetition_type in repetition_types" class="form-check" :id="repetition_type.id">
                <input v-if="invoicing_details" type="radio" name="repetition_type_review" class="form-check-input"
                    :id="'review_' + repetition_type.key" :value="repetition_type.id"
                    v-model="invoicing_details.review_repetition_type" />
                <label :for="'review_' + repetition_type.key" class="form-check-label">{{ repetition_type.display_name
                }}</label>
            </div>
        </div>
    </div>
    <div v-if="show_crown_land_rent_review_date">
        <CrownLandRentReviewDate v-if="invoicing_details" :review_dates="invoicing_details.crown_land_rent_review_dates" />
    </div>
    <div v-if="show_invoicing_frequency" class="row mb-2">
        <label for="invoicing_frequency" class="col-form-label col-sm-4">Invoicing Frequency</label>
        <div class="col-sm-8">
            <div class="d-flex align-items-center">
                <div class="pe-3">
                    <input v-if="invoicing_details" type="number" min="1" max="5" step="1" id="invoicing_once_every"
                        class="form-control" v-model="invoicing_details.invoicing_once_every">
                </div>
                <div class="pe-3">
                    time<span v-if="invoicing_details.invoicing_once_every > 1">s</span> per
                </div>
                <div class="">
                    <select class="form-select" aria-label="Repetition Type"
                        v-model="invoicing_details.invoicing_repetition_type">
                        <option disabled :value="null">Select a Frequency</option>
                        <option v-for="(repetition_type, i) in repetition_types" :value="repetition_type.id">{{
